...
func _ready() -> void:
	...
	create_phrase(0, "... \n(Можно нажать esc чтобы выйти из диалога)")
	...
	# Диалог монстра в уровне пещеры
	create_dialog(5)
	
	create_phrase(0, "Почему ты так на меня смотришь? \n(Можно нажать esc чтобы выйти из диалога)")
	create_choice("Кто ты?", 1)
	create_choice("Что ты здесь делаешь?", 2)
	create_choice("Где мы?", 3)
	
	create_phrase(1, "Поверь тебе лучше не знать...")
	create_choice("Далее", 0)
	
	create_phrase(2, "Эм... Я здесь живу вообще-то!")
	create_choice("Но здесь же опасно", 4)
	create_choice("Ты здесь один?", 5)
	
	create_phrase(4, "Ты про ловушки? Они наоборот делают это место безопасным, не многим людям удаётся добраться до сюда.")
	create_choice("Далее", 0)	
	
	create_phrase(5, "Нет, я здесь со своей семьёй. Но тебе их лучше не встречать. Они, в отличии от меня, не вегетарианцы...")
	create_choice("Далее", 0)
	
	create_phrase(3, "Ну во-первых мы в моём доме, а я кстати не звал гостей. А если более глобально, мы в магическом лесу.")
	create_choice("Далее", 0)
